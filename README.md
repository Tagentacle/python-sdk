# Tagentacle Python SDK (Archived)
...
Or with uv in your project:
```bash
uv add tagentacle-py-core
uv add tagentacle-py-mcp   # if you need MCP integration
```
...
The `example_ws/` in this repo contains a legacy 3-node example (agent + mcp_server + bringup). For the full 5-node chatbot system see [example-bringup](https://github.com/Tagentacle/example-bringup).
